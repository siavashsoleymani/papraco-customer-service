...
export const Offer = (props: IOfferProps) => {
  ...
  const { offerList, match, loading, totalItems } = props;
  return (
    <div>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('offerType')}>
                  Offer Type <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('notifInterval')}>
                  Notif Interval <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {offerList.map((offer, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{offer.notifInterval}</td>
                  <td>{offer.company ? <Link to={`company/${offer.company.id}`}>{offer.company.id}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${offer.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button
                        tag={Link}
                        to={`${match.url}/${offer.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${offer.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
