...
export interface IUserCommentProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const UserComment = (props: IUserCommentProps) => {
  ...
  const { userCommentList, match, loading, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {userCommentList && userCommentList.length > 0 ? (
            ...
            <tbody>
              {userCommentList.map((userComment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userComment.company ? <Link to={`company/${userComment.company.id}`}>{userComment.company.id}</Link> : ''}</td>
          ...
          !loading && <div className="alert alert-warning">No User Comments found</div>
        )}
      </div>
      {props.totalItems ? (
...
const mapStateToProps = ({ userComment }: IRootState) => ({
  userCommentList: userComment.entities,
  loading: userComment.loading,
  totalItems: userComment.totalItems,
});
