...
export const Staff = (props: IStaffProps) => {
  ...
  const { staffList, match, loading, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {staffList && staffList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('desciption')}>
                  Desciption <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {staffList.map((staff, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{staff.email}</td>
                  <td>{staff.desciption}</td>
                  <td>{staff.company ? <Link to={`company/${staff.company.id}`}>{staff.company.id}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${staff.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${staff.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Staff found</div>
        )}
      </div>
      {props.totalItems ? (
        <div className={staffList && staffList.length > 0 ? '' : 'd-none'}>
