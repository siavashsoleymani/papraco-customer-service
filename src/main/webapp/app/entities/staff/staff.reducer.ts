...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_STAFF: 'staff/PARTIAL_UPDATE_STAFF',
...
export default (state: StaffState = initialState, action): StaffState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_STAFF):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_STAFF):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_STAFF):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const getEntities: ICrudGetAllAction<IStaff> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IStaff>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
...
export const partialUpdate: ICrudPutAction<IStaff> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_STAFF,
